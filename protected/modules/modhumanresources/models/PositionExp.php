 ...
 */
class PositionExp extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @return PositionExp the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('employee_id, period_start, period_finish, position, job_description', 'required'),
			array('employee_id', 'length', 'max'=>20),
			array('position', 'length', 'max'=>100),
			array('job_description', 'length', 'max'=>200),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, employee_id, period_start, period_finish, position, job_description', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'position' => 'Jabatan',
			'job_description' => 'Deskripsi Jabatan',
		);
	}
